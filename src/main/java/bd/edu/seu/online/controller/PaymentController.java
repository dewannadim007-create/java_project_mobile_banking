...
public class PaymentController implements Initializable {
    ...
    @FXML private ChoiceBox<String> providerChoicebox,typeChoiceBox;

    @FXML private void changeToUtility( ) {HelloApplication.changeScene("utility");}
    @FXML private void add500() {
       amountField.setText("");
       amountField.setText("500");
   }
    @FXML private void add1000() {
        amountField.setText("");
        amountField.setText("1000");
    }
    @FXML private void add1500() {
        amountField.setText("");
        amountField.setText("1500");
    }
    @FXML private void add1200() {
        amountField.setText("");
    ...
    @FXML private void proceed( ) {
        ...
        String provider = providerChoicebox.getValue().toLowerCase();
        String type = typeChoiceBox.getValue();

        if(givenAmount< balance && UserService.verifyPin(passwordField.getText()) && TransactionServices.utilityAccountCheck(provider,account,type))
        {
    ...
    public void initialize(URL url, ResourceBundle resourceBundle) {
        ...
        observableList.add("DESCO");
        observableList.add("DPDC");
        providerChoicebox.setItems(observableList);
